...
echo ""
echo "---------------- modifying python to skip building OpenSSL ----------------"
echo ""
$VENV_BIN_DIR/python3 $SCRIPT_DIR/modify_build_installer.py
retVal=$?
if [ $retVal -ne 0 ]; then
    echo "Could not modify python to skip building OpenSSL!"
    exit $retVal
fi
...
echo ""
echo "---------------- Using pre-compiled OpenSSL package ----------------"
echo ""
$SCRIPT_DIR/use_precompiled_openssl.sh
retVal=$?
if [ $retVal -ne 0 ]; then
    echo "Could not use pre-compiled OpenSSL package!"
    exit $retVal
fi
